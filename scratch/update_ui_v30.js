...
fs.writeFileSync('scripts/xELU-v30.mjs', code, 'utf8');
    ...
    list.forEach(file => {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            if (content.includes('xELU-v29.mjs')) {
                content = content.replace(/xELU-v29\.mjs/g, 'xELU-v30.mjs');
                fs.writeFileSync(fullPath, content, 'utf8');
            }
        }
    });
}
// Delete the old one if we successfully created a new one
if (fs.existsSync('scripts/xELU-v30.mjs') && fs.readFileSync('scripts/xELU-v30.mjs', 'utf8') !== fs.readFileSync('scripts/xELU-v29.mjs', 'utf8')) {
    updateReferences('.');
    // Oh wait, my script is saving to xELU-v30.mjs, and deleting xELU-v29.mjs.
    // That means it increments.
    if (fs.existsSync('scripts/xELU-v29.mjs')) {
        fs.unlinkSync('scripts/xELU-v29.mjs');
    }
} else {
    // If we run this script repeatedly, xELU-v29.mjs won't exist anymore, it'll start with v30 and output to v30.
    // That's totally fine.
    fs.writeFileSync('scripts/xELU-v30.mjs', code, 'utf8');
}

console.log("Updated Step 1 (Strategy card) to Strategic Roadmap!");
